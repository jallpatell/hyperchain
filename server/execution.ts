...
export async function executeWorkflow(workflow: Workflow, executionId: number, triggerData?: any): Promise<void> {
    ...
    try {
        ...
        const broadcastProgress = (status: ExecutionStatus, error?: string): void => {
            const progress: ExecutionProgress = {
                executionId,
                workflowId: workflow.id,
                status,
        ...
        while (queue.length > 0) {
            ...
            try {
                ...
                console.error(`[executor] Execution ${executionId} failed at node ${currentNode.id}:`, errorMessage);
                return;
            }
        ...
        try {
            ...
            console.error(`[executor] Failed to update execution ${executionId} in storage:`, storageErr);
        }
    }
}
