    ...
    return (
      ...
      `}
        >
            ...
            {/* Icon badge */}
            <div className={`flex items-center justify-center w-8 h-8 rounded-lg shrink-0 ${meta.bgColor}`}>
                <Icon className={`w-4 h-4 ${meta.iconColor}`} />
            </div>

            {/* Label + type pill */}
            <div className="flex flex-col min-w-0">
                <span className="text-[13px] font-semibold text-gray-800 truncate leading-tight">{label}</span>
                <span className="flex items-center gap-1 mt-0.5">
                    <span className={`w-1.5 h-1.5 rounded-full ${meta.dotColor}`} />
                    <span className="text-[10px] text-gray-400 font-medium uppercase tracking-wide">{meta.type}</span>
                </span>
            </div>
        </div>
    );
...
function SidebarNodeItem({ node }: { node: (typeof NODE_TYPES)[number] }) {
    ...
    return (
      ...
      `}
        ...
        >
            <div className={`flex items-center justify-center w-7 h-7 rounded-lg shrink-0 ${node.bgColor}`}>
        ...
        <div className="flex flex-col h-screen w-full">
            ...
            <div className="flex items-center justify-between px-4 py-2 border-b bg-white shadow-sm z-10">
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-extrabold select-none text-[#EF486F] font-mono text-xl uppercase border border-1 p-2 rounded-lg cursor-default">
                        {workflowName}
                    </span>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <div className="w-52 border-r bg-gray-50 flex flex-col gap-1.5 p-3 overflow-y-auto shrink-0">
                    <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-widest mb-1 px-1">Nodes</p>
                    {NODE_TYPES.map((node) => (
                ...
                <div className="fixed bottom-4 right-4 bg-white border rounded-lg shadow-lg p-4 w-96 max-h-96 overflow-y-auto z-40">
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-semibold">Execution #{currentExecutionId}</span>
                        </div>
                        <span
                        ...
                        {executionState.progress.nodes.map((node) => (
                            <div key={node.nodeId} className="text-xs border rounded p-2 bg-gray-50">
                                <div className="flex items-center justify-between">
                                    <span className="font-mono font-semibold truncate">{node.nodeId}</span>
                                    <span
                                ...
                                {node.error && <p className="text-red-600 mt-1 text-xs">{node.error}</p>}
                                {node.output && (
