...
import { Plus, Search, MoreVertical, Play, Clock, GitBranch } from 'lucide-react';
import {
    DropdownMenu,
    DropdownMenuTrigger,
    DropdownMenuContent,
...
export default function Workflows() {
    ...
    // Helper function to get unique node types from a workflow
    const getWorkflowNodeTypes = (nodes: any[]) => {
        if (!nodes || nodes.length === 0) return [];
        
        const uniqueTypes = Array.from(new Set(nodes.map(node => node.type)));
        return uniqueTypes.map(type => {
            const nodeMeta = NODE_TYPES.find(n => n.type === type);
            return nodeMeta || { type, label: type, iconColor: 'text-gray-500' };
        });
    };

    // Helper function to format tags with limit
    const formatTags = (nodeTypes: any[]) => {
        if (nodeTypes.length === 0) return null;
        
        const displayTags = nodeTypes.slice(0, 5);
        const extraCount = nodeTypes.length - 5;
        
        return (
            <div className="flex flex-wrap gap-1.5 mt-3">
                {displayTags.map((nodeType, index) => {
                    const IconComponent = nodeType.icon;
                    return (
                        <span
                            key={index}
                            className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-muted ${nodeType.iconColor}`}
                ...
                {extraCount > 0 && (
                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-muted text-muted-foreground" title={`${extraCount} more node types`}>
                        +{extraCount}
                    </span>
                )}
            </div>
                ...
                <div className="flex-1 overflow-y-auto px-8 pb-8">
                    {isLoading ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {[1, 2, 3].map((i) => (
                                <div key={i} className="h-48 rounded-xl bg-card border border-border animate-pulse animate-blink" />
                            ))}
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredWorkflows?.map((workflow) => {
                                const nodeTypes = getWorkflowNodeTypes(workflow.nodes);
                                return (
                                <div
                                    key={workflow.id}
                                ...
                                >
                                    <div className="p-6 flex-1">
                                        <div className="flex items-start justify-between mb-4">
                                            <div
                                                className={`w-10 h-10 rounded-lg flex items-center justify-center ${workflow.isActive ? 'bg-green-100 text-green-600' : 'bg-muted text-muted-foreground'}`}
                                            >
